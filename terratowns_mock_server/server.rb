...
# You would never use a global variable in production server.
$home = {}
...
class Home
  ...
  # It's used as an ORM. It has a module within ActiveModel that provides validations.
  ...
  # This will set a getter and setter eg. 
  # home = new Home()
...
class TerraTownsMockServer < Sinatra::Base
  ...
  def find_user_by_bearer_token
    ...
    # If we cant find it than return an error or if it doesn't match code = access_code = token
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    # begin/resurce is a try/catch, if an error occurs, result it.
    begin
      # Sinatra does not automatically part json bodys as params like rails so we need to manuall parse it.
      payload = JSON.parse(request.body.read)
    rescue JSON::ParserError
      halt 422, "Malformed JSON"
    end

    # Assign the payload to variables to make easier to work with the code
    ...
    # Printing the variables out to console to make it eaiser to see or debug what we have inputed into this endpoint
    puts "name #{name}"
    ...
    puts "uuid #{uuid}"
    # Will mock our data to our mock databse which just a global variable
    $home = {
      uuid: uuid,
      name: name,
      town: town,
